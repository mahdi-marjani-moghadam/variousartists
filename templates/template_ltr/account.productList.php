                                    ...
                                    <div class="col-md-12 col-sm-12 col-xs-12" style="overflow: scroll">
                                        <table   class="table table-responsive table-striped">
                                            ...
                                            <tr>
                                                ...
                                                <td><?=$value['rate']?> From ( <?=$value['rate_count']?> ) Rate</td>
                                                <td>
                                                    <div class="style-msg   <?=($value['status'] == 1)?'':'style-msg-light';?>" style="background-color: <?=($value['status'] == 1)?'#AEE239':'#444';?>">
                                                        <div class="sb-msg"><i class="<?=($value['status'] == 1)?'icon-thumbs-up':'icon-remove';?>"></i> <?=($value['status'] == 1)?'Active':'Inactive';?></div>
                                                    </div>

                                                    <a href="<?=RELA_DIR?>account/editProduct/<?=$value['Artists_products_id']?>" class="button button-3d button-dirtygreen" >Edit</a>
                                                    <a href="<?=RELA_DIR?>account/deleteProduct/<?=$value['Artists_products_id']?>" class="button button-3d button-red" >Delete</a>
