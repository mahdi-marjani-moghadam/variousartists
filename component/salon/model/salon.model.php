...
class salonModel
{
     ...
     * @return mixed
     */
    public function getSalonUlLi($parent_id = 0)
    {
        $result = $this->getSalonTree();
    ...
    public function getSalonUlLiSearch($SalonTree,$parent_id = 0)
    {
        ...
        $mainMenu = $this->convertTreetoLiUlSearch($SalonTree[$parent_id], 1, $SalonTree);
        //$mainMenu = "<ul>\n".$mainMenu ."</ul>";
        $result['result'] = 1;
        $result['export']['list'] = $mainMenu;
        ...
        /*
                ...
                foreach ($array as $key => $val) {
                    ...
                    if (is_array($mainList[$val['Salon_id']])) {
                        ...
                        $mainMenu .= '<a class="hyperLink text-center" href="'.RELA_DIR.$cityStr.'company/'.(strlen($val['url']) ? $val['Salon_id'].'/'.$val['url'] : '#').'"><i class="fa fa-link"></i></a>';
                        ...
                        $mainMenu .= '<a class="link text-regular transition" href="'.RELA_DIR.$cityStr.'company/'.(strlen($val['url']) ? $val['Salon_id'].'/'.$val['url'] : '#').'">لیست همه '.$val['title'].'</a>';
                        $mainMenu .= '</h2>';
                        ...
                        $mainMenu .= '<a class="link text-right transition" href="'.RELA_DIR.$cityStr.'company/'.(strlen($val['url']) ? $val['Salon_id'].'/'.$val['url'] : '#').'">';
