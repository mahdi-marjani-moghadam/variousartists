...
class genreModel
{
    ...
    public function getGenreById($id)
    {
        ...
        $result = genreModelDb::getGenreById($id);
    ...
    public function convert($_input, $temp='', $space = '-')
    {
        ...
        foreach ($_input as $key => $val) {
            ...
            $mainMenu[$val['Genre_id']]['title'] = $val['title'];
            $mainMenu[$val['Genre_id']]['level'] = $this->level;

            $temp = $temp.$space;
            ++$this->level;
            if (isset($this->listCat[$val['Genre_id']])) {
                $this->convert($this->listCat[$val['Genre_id']], $temp, $space);
            }
            --$this->level;
            $len = strlen($space);
     ...
     * @return mixed
     */
    public function getGenreUlLi($parent_id = 0)
    {
        $result = $this->getGenreTree();
    ...
    public function getGenreUlLiSearch($GenreTree,$parent_id = 0)
    {


        $mainMenu = $this->convertTreetoLiUlSearch($GenreTree[$parent_id], 1, $GenreTree);
        //$mainMenu = "<ul>\n".$mainMenu ."</ul>";
        $result['result'] = 1;
        $result['export']['list'] = $mainMenu;
    ...
    public function convertTreetoLiUl($array, $root = 0, $all=[])
    {
        ...
        foreach ($array as $key => $val) {
            ...
            if (is_array($mainList[$val['Genre_id']])) {
                $mainMenu .= '
                    <li>
                        <a href="'.RELA_DIR.$cityStr.'artists/g-'.(strlen($val['url']) ? $val['Genre_id'].'/'.$val['url'] : '#').'">'.$val['title'].'</a>';
                $this->convertTreetoLiUl($mainList[$val['Genre_id']]);
                $mainMenu .= '</li>';
            } else {
                $mainMenu .= "\t".'
                    <li>
                ...
                $mainMenu .= '
                    <li>
                        <a class="company-name"><span>('.$val['count'].')</span>
                            <label for="genre-'.$val['Genre_id'] .'" class="company-name">'.$val['title'].
                                '<input type="checkbox" name="genre[]" id="genre-'.$val['Genre_id'] .'" value="'.$val['Genre_id'].'">
                            </label>
                        </a>';
                    $this->convertTreetoLiUlSearch($mainList[$val['Genre_id']]);
                $mainMenu .= '</li>';
        ...
        /*
                ...
                foreach ($array as $key => $val) {
                    ...
                    if (is_array($mainList[$val['Genre_id']])) {
                        ...
                        $mainMenu .= '<a class="hyperLink text-center" href="'.RELA_DIR.$cityStr.'company/'.(strlen($val['url']) ? $val['Genre_id'].'/'.$val['url'] : '#').'"><i class="fa fa-link"></i></a>';
                        ...
                        $mainMenu .= '<a class="link text-regular transition" href="'.RELA_DIR.$cityStr.'company/'.(strlen($val['url']) ? $val['Genre_id'].'/'.$val['url'] : '#').'">لیست همه '.$val['title'].'</a>';
                        $mainMenu .= '</h2>';
                        ...
                        $mainMenu .= '<a class="link text-right transition" href="'.RELA_DIR.$cityStr.'company/'.(strlen($val['url']) ? $val['Genre_id'].'/'.$val['url'] : '#').'">';
    ...
    public function allGenre()
    {
        ...
        include_once dirname(__FILE__).'/genre.model.db.php';
        $result = genreModelDb::getGenreAll();
        $this->list = $result['export']['list'];
        $this->recordsCount = $result['export']['recordsCount'];
    ...
    public function getGenreParents($parentId)
    {
        include_once dirname(__FILE__).'/genre.model.db.php';
        $result = genreModelDb::getGenreParents($parentId);
        $this->list = array_reverse($result['export']['list']);

        return $result;
    }

    public function getGenreChildes($genreId)
    {
        include_once dirname(__FILE__).'/genre.model.db.php';
        $result = genreModelDb::getGenreChildes($genreId);
        $this->list = $result['export']['list'];
